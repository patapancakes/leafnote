...
class PyResource(resource.Resource):
	...
	def UpdateThreaded(self, flipnotes):#run in an another thread
		...
		if self.pages:#not on startup
			print time.strftime("[%H:%M:%S] Updated flipnotes.ugo")
		self.pages = pages
	def MakePage(self, flipnotes, page, next, count):
		...
		ugo.Items.append(("topscreen text", ["All Flipnotes", "Flipnotes", str(count), "", "All Flipnotes submitted to Leafnote."], 0))
		...
		ugo.Items.append(("unknown", ("3", "http://flipnote.hatena.com/ds/v2-xx/post.uls", "UABvAHMAdAAgAEYAbABpAHAAbgBvAHQAZQA=")))
		
		#previous page
		if page > 1:
			ugo.Items.append(("button", 115, "Previous", "http://flipnote.hatena.com/ds/v2-xx/frontpage/flipnotes.uls?page=%i" % (page-1), ("", ""), None))
		...
		#next page
		if next:
			ugo.Items.append(("button", 115, "Next", "http://flipnote.hatena.com/ds/v2-xx/frontpage/flipnotes.uls?page=%i" % (page+1), ("", ""), None))
